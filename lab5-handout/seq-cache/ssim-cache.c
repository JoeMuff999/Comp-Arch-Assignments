...
static void update_state()
{
    ...
    if (mem_write) {
      /* Should have already tested this address */
        set_word_cache(mem, mem_addr, mem_data);
...
/*
    For this seq-cache, copy in your sim_step() function from seq
    and replace the following
    set_word_val -> set_word_cache
    set_byte_val -> set_byte_cache
    get_word_val -> get_word_cache
    get_byte_val -> get_byte_cache
 */
static byte_t sim_step()
{
    ...
    // reg_id_t reg_A = REG_NONE;
    // reg_id_t reg_B = REG_NONE;
    ...
    byte0_valid = get_byte_cache(mem, valp, &byte0);
    valp++;
    icode = HI4(byte0);
    ifun = LO4(byte0);
    ...
    if(need_regids)
    {
        byte1_valid = get_byte_cache(mem, valp, &byte1);
        valp++;
        ra = HI4(byte1);
        rb = LO4(byte1);
    ...
    if(need_valc){
        word_valid = get_word_cache(mem, valp, &valc);
        valp += 8;
    }
    ...
    // {
    //     case '0x2':
    //         byte_t reg_byte = get_byte_val(pc+1);
    //         ra = HI4(reg_byte);
    //         rb = LO4(reg_byte);
    //         valp = pc+2;
    //         break;
    ...
    switch(icode)
    {
        ...
        break;
        case I_MRMOVQ:
        if(get_word_cache(mem,vale,&valm) == FALSE)
        {
            return STAT_ADR;
        }
        break;
        ...
        break;
        case I_RET:
        if(get_word_cache(mem,vala,&valm) == FALSE)
        {
            return STAT_ADR;
        }
        break;
    ...
    {
        byte_t OOB_dummy = 0;
        bool_t check_OOB = get_byte_cache(mem, mem_addr, &OOB_dummy);
